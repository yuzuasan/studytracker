...
/**
 * StudyRecordCreateRequestの単体テスト
 */
@DisplayName("StudyRecordCreateRequest 単体テスト")
class StudyRecordCreateRequestTest {

    private Validator validator;
    private StudyRecordCreateRequest request;

    @BeforeEach
    void setUp() {
        validator = Validation.buildDefaultValidatorFactory().getValidator();
        request = new StudyRecordCreateRequest();
    }
    ...
    class ValidationTests {
        ...
        void validRequest_NoViolations() {
            ...
            // 実行
            Set<ConstraintViolation<StudyRecordCreateRequest>> violations = validator.validate(request);
        ...
        @Test
        @DisplayName("異常系：dateがnullの場合、バリデーションエラー")
        void nullDate_ValidationError() {
            // 準備
            ...
            assertThat(violations.iterator().next().getMessage()).isEqualTo("dateは必須です");
        }

        @Test
        @DisplayName("異常系：subjectがnullの場合、バリデーションエラー")
        void nullSubject_ValidationError() {
            // 準備
            TestUtil.setField(request, "date", LocalDate.of(2024, 1, 15));
            ...
            // 検証
            assertThat(violations).hasSize(1);
            assertThat(violations.iterator().next().getMessage()).isEqualTo("subjectは必須です");
        }
        ...
        void subjectTooLong_ValidationError() {
            ...
            // 検証
            assertThat(violations).hasSize(1);
            assertThat(violations.iterator().next().getMessage()).isEqualTo("subjectは100文字以内で入力してください");
        }
        ...
        @Test
        @DisplayName("異常系：studyMinutesがnullの場合、バリデーションエラー")
        void nullStudyMinutes_ValidationError() {
            // 準備
            TestUtil.setField(request, "date", LocalDate.of(2024, 1, 15));
            TestUtil.setField(request, "subject", "Java");
            ...
            // 検証
            assertThat(violations).hasSize(1);
            assertThat(violations.iterator().next().getMessage()).isEqualTo("studyMinutesは必須です");
        }
